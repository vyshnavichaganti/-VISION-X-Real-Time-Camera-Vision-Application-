...
class VisionService:
    ...
    def __init__(self):
        ...
        self.segmentor: Optional[BaseSegmentor] = None
        self.depth_model: Optional[BaseDepthModel] = None
    ...
    def initialize(self) -> None:
        """
        Loads detector, SAM segmentor, MiDaS depth model, and distance calibration on startup.
        Logs individual component status clearly.
        """
        if self.detector is not None and self.detector.is_loaded:
            logger.info("Vision Service detector is already initialized.")
            return

        logger.info("Initializing Vision Service AI components...")
        ...
        if settings.SEGMENTATION_ENABLED:
            try:
                ...
                logger.warning(f"Failed to initialize FastSAM segmentor (falling back): {seg_err}")
                self.segmentor = None
                logger.info("Segmentor: UNAVAILABLE")
        else:
        ...
        if settings.DEPTH_ENABLED:
            try:
                self.depth_model = MiDaSDepthModel(
                ...
                logger.warning(f"Failed to initialize MiDaS depth model (falling back): {depth_err}")
                self.depth_model = None
                logger.info("Depth: UNAVAILABLE")
        else:
        ...
        logger.info("Vision Service initialization complete.")
    ...
    def detect_objects(
        ...
        try:
            ...
            # Step 3: SAM Segmentation Pass
            segmentation_time_ms = 0.0
            if enable_segmentation and settings.SEGMENTATION_ENABLED and self.segmentor is not None and len(tracked_detections) > 0:
                try:
                    tracked_detections, segmentation_time_ms = self.segmentor.segment(image, tracked_detections)
                except Exception as seg_exc:
                    logger.warning(f"SAM Segmentation pass error (graceful fallback): {seg_exc}")

            # Step 4: Monocular Depth Estimation Pass
            depth_time_ms = 0.0
            depth_summary = DepthSummarySchema(available=False)
            if enable_depth and settings.DEPTH_ENABLED and self.depth_model is not None and len(tracked_detections) > 0:
                try:
                    tracked_detections, depth_summary, depth_time_ms = self.depth_model.estimate_depth(image, tracked_detections)
